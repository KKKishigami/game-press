      ...
      <div className="max-w-[1200px] mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-xs text-gray-500 mt-2">※記事にはアフィリエイト広告が含まれる場合があります。</p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-xs">
              {['PS5', 'PS4', 'Switch', 'Xbox', 'PC'].map((p) => (
                <li key={p}><Link href={`/platform/${p}`} className="hover:text-white hover:underline transition-colors">{p}</Link></li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-xs">
              {[['2020s','2020年代'],['2010s','2010年代'],['2000s','2000年代'],['1990s','1990年代'],['1980s','1980年代']].map(([id, label]) => (
                <li key={id}><Link href={`/era/${id}`} className="hover:text-white hover:underline transition-colors">{label}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-xs">
              {[['プライバシーポリシー','/privacy-policy'],['利用規約','/terms'],['広告・PR開示','/disclosure'],['お問い合わせ','/contact']].map(([label, href]) => (
                <li key={label}><Link href={href} className="hover:text-white hover:underline transition-colors">{label}</Link></li>
              ))}
            </ul>
        ...
        <div className="border-t border-gray-700 pt-4 flex flex-col md:flex-row items-center justify-between gap-2">
          ...
          <p className="text-xs text-gray-600 text-center">掲載の画像は公式プレスキット・許諾素材を使用。各権利は権利者に帰属します。</p>
        </div>
      </div>
    </footer>
  );
