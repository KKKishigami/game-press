...
export default async function ArticlePage({ params }: Props) {
  ...
  const renderBody = (body: string) => {
    return body.split('\n\n').map((block, i) => {
      ...
      if (block.startsWith('### ')) {
        return <h3 key={i} className="text-sm font-bold text-gray-800 mt-4 mb-2 border-b border-gray-300 pb-1">{block.replace('### ', '')}</h3>;
      }
      if (block.startsWith('> ')) {
              ...
              {lines.map((row, j) => {
                ...
                return (
                  <tr key={j} className={j === 0 ? 'bg-gray-700 text-white font-bold' : 'border-t border-gray-200 even:bg-gray-50'}>
                    {cells.map((cell, k) => <td key={k} className="px-2 py-1.5">{cell.trim()}</td>)}
                  </tr>
                );
              })}
            </table>
      ...
      return <p key={i} className="text-sm text-gray-700 leading-relaxed my-2">{block}</p>;
    });
  };
      ...
      <div className="flex gap-3">
        <article className="flex-1 min-w-0">
          <nav className="flex items-center gap-1 text-xs text-gray-500 mb-2">
            ...
            <span>›</span>
            <Link href={`/category/${article.category}`} className="hover:text-[#cc0000]">{categoryLabel(article.category)}</Link>
            <span>›</span>
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex flex-wrap gap-1.5 mb-2">
              <span className="bg-[#cc0000] text-white text-xs font-bold px-2 py-0.5">{categoryLabel(article.category)}</span>
              <span className="bg-gray-700 text-white text-xs font-bold px-2 py-0.5">{articleTypeLabel(article.type)}</span>
              {article.type === 'interview_summary' && (
                <span className="border border-orange-400 text-orange-600 text-xs px-2 py-0.5">引用記事</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 mb-3 pb-2 border-b border-gray-200 flex-wrap">
              ...
              <span className="text-gray-400 text-[10px]">※本記事にはアフィリエイト広告が含まれる場合があります</span>
            </div>
            ...
            <div className="mb-4"><AdBanner label="広告 (記事上)" height="h-14" /></div>
            ...
            {article.youtubeUrl && (
              <div className="mt-4">
                ...
                <div className="relative w-full aspect-video bg-gray-200">
                  <iframe src={article.youtubeUrl} title="YouTube動画"
            ...
            <div className="mt-4 pt-3 border-t border-gray-200">
              <div className="flex flex-wrap gap-1.5 mb-2">
                {article.tags.map((tag) => (
                  <span key={tag} className="text-[10px] px-2 py-0.5 bg-gray-100 border border-gray-300 text-gray-600 hover:border-[#cc0000] hover:text-[#cc0000] cursor-pointer transition-colors">#{tag}</span>
                ))}
              </div>
              <div className="flex flex-wrap gap-1.5">
                ...
                {article.platforms.map((p) => (
                  <Link key={p} href={`/platform/${p}`}
                    className="text-[10px] px-2 py-0.5 bg-gray-700 text-white hover:bg-[#cc0000] transition-colors">{p}</Link>
                ))}
              </div>
            </div>

            <div className="mt-4"><AdBanner label="広告 (記事下)" height="h-14" /></div>
            <p className="text-gray-400 text-[10px] mt-2">※掲載画像は公式プレスキット・許諾素材を使用しています。各権利は権利者に帰属します。</p>
          </div>
          ...
          <div className="mt-3 bg-white border border-gray-200 p-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {related.slice(0, 3).map((r) => <ArticleCard key={r.id} article={r} size="medium" />)}
            </div>
          </div>
        </article>
        ...
        <aside className="hidden md:block w-52 flex-shrink-0 space-y-3">
          ...
          <div className="bg-white border border-gray-200">
            ...
            <div className="p-2 divide-y divide-gray-100">
              {related.map((r) => <ArticleCard key={r.id} article={r} size="list" />)}
            </div>
          </div>
